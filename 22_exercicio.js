...
const funcaoDaSorte = (valor) => {
    ...
    let flutuante = Math.floor(Math.random() * 4 + 1)
    
   /* if (valor == flutuante) {
        return `Parabéns! O número sorteado foi o ${flutuante}`
    } else return `Que pena! O número sorteado foi o ${flutuante}`

    return (valor == flutuante ? `Parabéns!`: `Que pena!`)+ `O número sorteado foi o ${flutuante}`*/
